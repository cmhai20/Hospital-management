...
private:
    ...
    std::map<std::string,Person*> total_patients_data;  //Store all the patients
                                                        //including past and current
    //Track all care periods for each patient
    std::map<std::string, std::vector<CarePeriod*>> patients_care_record;

    //Track care periods that staffs have participated in
    std::map<std::string, std::vector<CarePeriod*>> staffs_care_record;

    //Associate patients ID with their medicine
    std::map<std::string, std::vector<std::string>> prescribed_medicine();
};
